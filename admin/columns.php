...
add_filter( 'manage_edit-cars_for_sale_columns', 'cars_for_sale_columns' );
add_action( 'quick_edit_custom_box',  'max_add_quick_edit', 10, 2 );

function cars_for_sale_columns( $columns ) {
...
function manage_cars_for_sale_columns( $column ) {
	...
	if ( defined( 'CD_CUSTOM_NO_PHOTO' ) ) {
		$car_image = '<img width="75" src="' . $car_pic . '" onerror="ImgError(this, \'' . CD_CUSTOM_NO_PHOTO . '\');" /><br />(' . $count . ')';		
	} else {
		$car_image = '<img width="75" src="' . $car_pic . '" onerror="ImgError(this, \'' . $car_demon_pluginpath . 'images/no_photo.gif\');" /><br />(' . $count . ')';
	}
	...
	if ( "ID" == $column ) {
		...
		echo '<div id="show_sold_' . $post_id . '">
				<select id="is_sold_' . $post_id . '" onchange="update_car_sold(' . $post_id . ', this, \'sold\')">
					<option value="No"' . ($sold == 'No' || $sold == 'no' ? ' selected' : '' ) . '>' . __( 'No', 'car-demon' ) . '</option>
					<option value="Yes"' . ($sold == 'Yes' ? ' selected' : '' ) . '>' . __( 'Yes', 'car-demon' ) . '</option>
				</select>
			</div>';
	}
}
