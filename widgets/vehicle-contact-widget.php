<?php
include( 'vehicle-contact-widget-handler.php' );
...
class car_demon_vehicle_contact_Widget extends WP_Widget {
	...
	function car_demon_vehicle_contact_Widget() {
		...
		/* Create the widget. */
		parent::__construct( 'car_demon_vehicle_contact-widget', __( 'Car Demon Vehicle Contact', 'car-demon' ), $widget_ops, $control_ops );
	}
	/**
	...
	function widget( $args, $instance ) {
		...
		if ( $post_type == 'cars_for_sale' ) {
			if (is_single($post_id)) {
				...
				wp_register_script( 'car-demon-contact-widget-js', plugins_url() . '/car-demon/widgets/js/car-demon-vehicle-contact-widget.js', array(), CAR_DEMON_VER );
				$validate_phone = 0;
				...
				wp_localize_script( 'car-demon-contact-widget-js', 'cdContactWidgetParams', array( 
					...
					'form_js' => apply_filters('car_demon_mail_hook_js', $x, 'contact_us_vehicle', 'unk' ),
					'form_data' => apply_filters('car_demon_mail_hook_js_data', $x, 'contact_us_vehicle', 'unk' ),
...
function car_demon_display_vehicle_contacts( $post_id, $list_phone, $cc, $send_receipt, $send_receipt_msg ) {
	...
	$phone_contact = '';
	if ( $list_phone == 'Yes' ) {
		$phone_contact = '<h4>' . __( 'For more information','car-demon' ) . '<br />' . __( 'Call', 'car-demon' ) . ' ' . $contact_sales_name . ' ' . __( 'at:','car-demon' ) . '<br />' . $contact_sales_phone . '</h4>';
	}
	if ( isset( $car_demon_options['validate_phone'] ) ) {
		if ( $car_demon_options['validate_phone'] == 'Yes' ) {
	...
	$x = $phone_contact . '
		...
		<form enctype="multicontact/form-data" action="?send_contact=1" method="post" class="cdform contact-appointment " id="contact_form">
			...
			<input type="hidden" name="vehicle_photo" id="vehicle_photo" value="' . cd_main_photo( $post_id, 'thumbnail' ) . '" />
			<input type="hidden" name="nonce" id="nonce" value="' . $nonce . '" />
			<fieldset class="cd-fs1">
			<legend>' . __( 'Your Information', 'car-demon' ) . '</legend>
			<ol class="cd-ol">
				<li id="li-name" class=""><label for="cd_field_2"><span>' . __( 'Your Name', 'car-demon' ) . '</span></label><input type="text" name="cd_name" id="cd_name" class="single fldrequired" value="' . __( 'Your Name', 'car-demon' ) . '" onfocus="clearField(this)" onblur="setField(this)"><span class="reqtxt">*</span></li>
				<li id="li" class=""><label for="cd_field_"><span>' . __( 'Phone #', 'car-demon' ) . '</span></label><input type="text" name="cd_phone" id="cd_phone" class="single fldrequired" value="" ' . $validate_phone . '><span class="reqtxt">*</span></li>
				<li id="li-4" class=""><label for="cd_field_4"><span>' . __( 'Email', 'car-demon' ) . '</span></label><input type="text" name="cd_email" id="cd_email" class="single fldemail fldrequired" value=""><span class="emailreqtxt">*</span></li>
			</ol>
			</fieldset>
	';
	$x2 = '
			<fieldset class="cd-fs2">
				<legend>' . __( 'I have a Trade', 'car-demon' ) . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<select>
						<option></option>
			...
			<fieldset class="cd-fs2">
				<legend>' . __( 'I need Financing', 'car-demon' ) . '&nbsp;&nbsp;
					<select>
						<option></option>
						<option value="Yes">' . __( 'Yes', 'car-demon' ) . '</option>
						<option value="No">' . __( 'No', 'car-demon' ) . '</option>
	...
	$x .='
			...
			<legend>' . __( 'Questions or Comments', 'car-demon' ) . '</legend>
			<ol class="cd-ol">
				<li id="li-5" class=""><textarea name="contact_needed" id="contact_needed" class="contact_us_comment fldrequired"></textarea><br /><span class="reqtxt reqtxt_comment"><br />' . __( '* required', 'car-demon' ) . '</span></li>
			</ol>
			</fieldset>';
	$x = apply_filters( 'cd_form_filter', $x, 'contact_us_vehicle', $vehicle_location_slug );
	$x = apply_filters( 'car_demon_mail_hook_form', $x, 'contact_us_vehicle', $vehicle_location_slug ); //= deprecated
	$x .= '<p class="cd-sb"><input type="button" name="search_btn" id="sendbutton" class="search_btn contact_us_btn" value="' . __( 'Send Now!', 'car-demon' ) . '" onclick="return car_demon_validate()"></p></form>
		';
	return $x;
}
?>